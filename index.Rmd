...
When we want to consider the size of paygap between WNBA players and NBA players, we want to account for these kinds of factors, like how much playtime the average player gets and how many games the average player plays versus how many the average player starts, which inform our understanding of these pay differences across time and other variables. 
...
We can see that for every metric, NBA players are compensated in far greater amounts for each performance statistic than WNBA players, but it does vary in terms of the gap. When we want to consider these differences, It helps to get an idea of how much players are getting compensated on average, whether they are contributing greatly to their team or not. 
=======
...
plot, we can gain a better evaluate the extent extent of the gender pay
gap within professional basketball leagues.
...
``` {r}        
source("Chart3_yeartrends.R") plot(wnba_nba_year_trend_graph)
```
